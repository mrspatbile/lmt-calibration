...
original opening balance. Investor ownership is tracked in units. The
redeemable balance excludes units already requested and carried as deferred
backlog, which prevents the same units from generating another redemption
request while an earlier instruction remains outstanding.
...
At month end, paid units leave investor ownership. Deferred units remain owned
and economically invested, but are excluded from the balance available for new
redemption instructions. Therefore, for each investor class:

```text
closing_units = opening_units - paid_redemption_units

closing_redeemable_units = closing_units - deferred_backlog_units
```
...
Unpaid or deferred redemption demand is carried forward as units, not as a
fixed cash claim.

Each backlog entry records investor class, originating month, remaining units,
and NAV per unit at deferral. The deferral NAV is audit metadata; it does not
freeze the later execution price.

Each month, eligible backlog is valued at the current NAV per unit and added to
new cash-equivalent redemption demand:

```text
backlog_cash_value = remaining_units × current_nav_per_unit
```

Execution occurs at the execution-month NAV per unit. Paid backlog units reduce
investor ownership in that month. Remaining units retain their origin and
deferral metadata and continue to participate in NAV changes.
...
Swing recovery and liquidity-cost allocation apply only to paid redemptions.

Deferred units do not generate swing recovery, a swing-pricing receivable, or a
liquidity-cost allocation until they execute in a later month.
...
Deferred backlog is not multiplied again by behavioural feedback. Backlog units
carry forward unchanged until execution and are revalued at the current NAV per
unit when they join new monthly redemption demand. The current month’s
behavioural feedback multiplier applies only to that new demand.
