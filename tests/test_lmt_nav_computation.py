...
def test_nav_after_lmt_includes_swing_recovery():
    """Verify NAV after LMT includes recovery when simulated swing activation occurs."""
    sample_data = load_app_sample_data(Path("data/sample"))
    runs = run_scenario_across_market_conditions(
        sample_data, fund_id="lux_dynamic_allocation", strategy_id="cash_then_liquid_assets"
    )
...
def test_nav_after_lmt_uses_paid_redemption_only():
    ...
    for run in runs:
        ...
        # If the simulated gate activation condition is met, deferred should be positive.
...
def test_lmt_pills_reflect_simulated_activation_states():
    """Verify LMT pills correctly show simulated activation and buffer-breach states."""
    sample_data = load_app_sample_data(Path("data/sample"))
    runs = run_scenario_across_market_conditions(
        sample_data, fund_id="lux_dynamic_allocation", strategy_id="cash_then_liquid_assets"
    )
    ...
    for run in runs:
        ...
        # All simulated activation and breach states should be booleans.
...
def test_before_and_after_lmt_nav_consistency():
    ...
    for run in runs:
        ...
        # If neither simulated activation condition is met:
        ...
        if not run.lmt_activation.swing_activated and not run.lmt_activation.gate_activated:
            assert nav_after_lmt == nav_before_lmt, (
                f"Without simulated LMT effects, NAV should be same. Before: {nav_before_lmt}, After: {nav_after_lmt}"
            )

        # Simulated swing activation adds positive recovery to NAV after LMT effects.
        if run.lmt_activation.swing_activated:
            assert nav_after_lmt > nav_before_lmt, (
        ...
        # Simulated gate activation defers redemption and increases NAV after LMT effects.
        if run.lmt_activation.gate_activated:
